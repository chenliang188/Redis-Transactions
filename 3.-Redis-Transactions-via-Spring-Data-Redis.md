...
```java
    ...
    // A new value, newVal, is available at this point
    // We'll check if it is also the new max
    long max = valueOps.get(maxKey);
    if (max < newVal) {
        old = valueOps.getAndSet(maxKey, newVal);
    }
    
    int retryCount = 0;
    final int retryLimit = 5;
    while (max > newVal && retryCount < retryLimit) {
        max = valueOps.getAndSet(maxKey, newVal);
        retryCount++;
    }
    if (max < newVal) {
        throw new RuntimeException("Updating max with value "
                + newVal + " for key " + maxKey + " failed.");
    }
```
